...
Хэштеги: #HaoriVision #процесс #behindthescenes. Формат: HTML.`,
  ...
  async handleUpdate(update) {
    ...
    // Обработка данных из Web App (miniapp)
    if (update.message.web_app_data) {
      try {
        const orderData = JSON.parse(update.message.web_app_data.data);
        baseLogger.info(
          { chatId, orderId: orderData.id },
          "Order received from miniapp",
        );

        let reply = `<b>Заказ получен!</b>\n\n`;
        reply += `<b>ID:</b> <code>${orderData.id || "—"}</code>\n`;
        if (orderData.productName)
          reply += `<b>Товар:</b> ${orderData.productName}\n`;
        if (orderData.size) reply += `<b>Размер:</b> ${orderData.size}\n`;
        if (orderData.price) reply += `<b>Цена:</b> ${orderData.price}\n`;
        if (orderData.notes) reply += `<b>Примечания:</b> ${orderData.notes}\n`;
        ...
        // Уведомление админу
        const adminMsg =
          `<b>Новый заказ из miniapp!</b>\n\n` +
          `<b>ID:</b> <code>${orderData.id || "—"}</code>\n` +
          `<b>User:</b> ${orderData.username ? "@" + orderData.username : chatId}\n` +
          (orderData.productName
            ? `<b>Товар:</b> ${orderData.productName}\n`
            : "") +
          (orderData.size ? `<b>Размер:</b> ${orderData.size}\n` : "") +
          (orderData.price ? `<b>Цена:</b> ${orderData.price}\n` : "") +
          (orderData.notes ? `<b>Примечания:</b> ${orderData.notes}\n` : "");
        await this.notifyAdmin(adminMsg);
      } catch (err) {
        baseLogger.error({ err, chatId }, "Failed to process web_app_data");
        await this.sendDirectMessage(
          chatId,
          "Заказ получен! Мы свяжемся с вами в ближайшее время.",
        );
      }
      return;
    }
  ...
  /**
   * Уведомление админа (отправка в TELEGRAM_ADMIN_CHAT_ID или TELEGRAM_CHANNEL_ID)
   */
  async notifyAdmin(text) {
    const adminChatId = process.env.TELEGRAM_ADMIN_CHAT_ID || this.channelId;
    if (!adminChatId) return;
    try {
      await this.sendDirectMessage(adminChatId, text);
    } catch (err) {
      baseLogger.warn({ err }, "Failed to notify admin via Telegram");
    }
  }
